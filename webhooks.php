...
if (!is_null($events['events'])) {
	...
	foreach ($events['events'] as $event) {
		...
		if ($event['type'] == 'message' && $event['message']['text'] == 'services') {
			...
			$jservice['contents']['hero']['url'] = 'https://lh3.googleusercontent.com/proxy/-1c7kpqSBd9MstpLoL1SssBqYrwCIFRJEO0GHycFDq7ZfioJKWjNkF1Qn7jex6Z18Kr51k_W2Oa5vHimHAIDDMVdGQbvjHHu6tKcXSWQxfuOT8NSvjOfZGTi1VR8e9aYNA_y';
			$jservice['contents']['body']['contents'][0]['text'] = 'นวดฝ่าเท้า';
			$jservice['contents']['body']['contents'][1]['text'] = 'จ อ พฤ ศ';
			$jservice['contents']['body']['contents'][2]['text'] = '9,999 ฿';
			$messages = $jservice;
		}
		...
		if ($event['type'] == 'message' && $event['message']['text'] == 'mybooking') {
			$messages = json_decode(file_get_contents("mybooking.json"),true);
		}
